...
class AppConfig:
    ...
    def load_from_env(cls) -> 'AppConfig':
        """从环境变量加载配置"""
        # 内置服务商配置 - 支持更多 NewAPI/OneAPI 兼容平台
        providers = {
            ...
            'agentrouter': ProviderConfig(
                ...
                login_path='/login',
                sign_in_path=None,  # AgentRouter 不支持独立签到接口，自动签到
                user_info_path='/api/user/self',  # 使用用户信息接口（需要cookie）
                api_user_key='new-api-user',
                bypass_method=None,  # 手动提供的cookie足够，无需WAF绕过
                waf_cookie_names=None,
            ),
            # 新增常用服务商的内置配置
            'newapi': ProviderConfig(
                name='newapi',
                domain='https://newapi.co',
                login_path='/login',
                sign_in_path='/api/user/sign_in',
                user_info_path='/api/user/self',
                api_user_key='new-api-user',
            ...
            'oneapi': ProviderConfig(
                name='oneapi',
                domain='https://oneapi.example.com',
                login_path='/login',
                sign_in_path='/api/user/sign_in',
                user_info_path='/api/user/self',
                api_user_key='new-api-user',
            ...
            'aipro': ProviderConfig(
                name='aipro',
                domain='https://aipro.com',
                login_path='/login',
                sign_in_path='/api/user/sign_in',
                user_info_path='/api/user/self',
                api_user_key='new-api-user',
            ...
            'openai2d': ProviderConfig(
                name='openai2d',
                domain='https://openai2d.com',
                login_path='/login',
                sign_in_path='/api/user/sign_in',
                user_info_path='/api/user/self',
                api_user_key='new-api-user',
...
class AccountConfig:
    ...
    name: str | None = None
    api_user: str | None = None  # 可选，为了兼容性
    api_user_header: str | None = None  # 可选，自定义请求头
    ...
    def from_dict(cls, data: dict, index: int) -> 'AccountConfig':
        ...
        # 支持多种 api_user 字段名
        api_user = data.get('api_user') or data.get('api_user_id') or data.get('user_id')
        ...
        return cls(
            ...
            api_user_header=data.get('api_user_header')
        )
    ...
    try:
        ...
        for i, account_dict in enumerate(accounts_data):
            ...
            if 'cookies' not in account_dict:
                print(f'ERROR: Account {i + 1} missing required cookies field')
                return None
