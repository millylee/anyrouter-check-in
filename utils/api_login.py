...
（如 sotamodel）。相比启动浏览器，直接调用 /api/user/login 更快也更稳定。
"""
...
def login_via_api(
	...
	*,
	...
	"""调用 JSON 登录接口获取 session cookies 与 api_user。
	...
	try:
		with httpx.Client(**client_kwargs) as client:
			...
			if not cookies:
				print(f'[FAILED] {account_name}: Login succeeded but no cookies were set')
				return None

			api_user = _extract_user_id(payload)
			if not api_user:
				# 少数站点登录响应不带用户对象，再查一次 /api/user/self 补齐
				api_user = _fetch_api_user(client, provider_config, headers)
			...
			return LoginResult(cookies=cookies, api_user=api_user)
...
def _fetch_api_user(client: httpx.Client, provider_config, headers: dict) -> str | None:
	"""登录响应缺少用户 id 时，回查用户信息接口"""
	try:
		response = client.get(f'{provider_config.domain}{provider_config.user_info_path}', headers=headers)
		if response.status_code != 200:
			return None
